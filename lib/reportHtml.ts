  ...
  <script>
    ...
    if (pvCharts?.dailyStorageChart?.length) {
      const dailyStorageTraces = [
        { type: 'bar', name: 'Dagelijkse opslagbehoefte', x: pvCharts.dailyStorageChart.map(d => d.date), y: pvCharts.dailyStorageChart.map(d => d.dailyStorageNeedKwh), marker: { color: '#A7D397', opacity: 0.72 } },
        { type: 'scatter', mode: 'lines', name: 'P50', x: pvCharts.dailyStorageChart.map(d => d.date), y: pvCharts.dailyStorageChart.map(d => d.p50), line: { color: '#2563EB', width: 3, dash: 'solid' } },
        { type: 'scatter', mode: 'lines', name: 'P75', x: pvCharts.dailyStorageChart.map(d => d.date), y: pvCharts.dailyStorageChart.map(d => d.p75), line: { color: '#DC2626', width: 3.5, dash: 'dash' } },
        { type: 'scatter', mode: 'lines', name: 'P90', x: pvCharts.dailyStorageChart.map(d => d.date), y: pvCharts.dailyStorageChart.map(d => d.p90), line: { color: '#7C3AED', width: 3, dash: 'dot' } }
      ];
...
export function generateInteractiveReportHtml(payload: PdfPayload): string {
  if (payload.analysisType === 'PV_SELF_CONSUMPTION' && payload.annualBill) {
    const startedAt = performance.now();
    const traceId = payload.annualBill.input.traceId;
    logAnnualBill('report.started', traceId, { recommendedBatteryKwh: payload.annualBill.advice.recommendedBatteryKwh });
    try {
    const capacity = payload.annualBill.advice.recommendedBatteryKwh;
    // Match the actual annual-bill recommendation, never the synthetic interval sizing.
    const brochure = capacity == null ? null : getRecommendedBrochureInfo({
      ...payload,
      sizing: { ...payload.sizing, recommendedProduct: {
        ...payload.sizing.recommendedProduct,
        label: `${capacity} kWh`, capacityKwh: capacity, powerKw: 0,
        unitCapacityKwh: capacity, breakdown: undefined
      } as NonNullable<PdfPayload['sizing']['recommendedProduct']> }
    });
    logAnnualBill('report.brochure.selected', traceId, { capacityKwh: capacity, found: Boolean(brochure), brochureKey: brochure?.key }, brochure ? 'info' : 'warn');
    const html = generateAnnualBillReportHtml(payload.annualBill, getEmbeddedLogoSrc(), brochure);
    logAnnualBill('report.completed', traceId, { htmlCharacters: html.length, durationMs: Math.round(performance.now() - startedAt) });
    return html;
    } catch (error) {
      logAnnualBill('report.failed', traceId, annualBillErrorDetails(error), 'error');
      throw error;
    }
  }
