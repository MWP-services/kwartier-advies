...
## 11) PV analysis flow
De app ondersteunt nu ook nalysisType = PV_SELF_CONSUMPTION.
In die modus blijft dezelfde parsing- en normalisatiepipeline actief, maar de analyse kijkt niet primair naar contractoverschrijdingen. In plaats daarvan wordt de batterij gebruikt om PV-surplus op te slaan en later terug te leveren aan de load.

### 11.1 Hoe pvKwh wordt gebruikt
Per 15-minuteninterval gebruikt de app:
- consumptionKwh als load
- pvKwh als lokale PV-opwek
- exportKwh als gemeten netinjectie, als die beschikbaar is

De flow per interval is:
- directe zelfconsumptie = min(consumptionKwh, pvKwh)
- PV-surplus = exportKwh als die kolom aanwezig is, anders max(0, pvKwh - directe zelfconsumptie)
- load-deficit = max(0, consumptionKwh - directe zelfconsumptie)

Daardoor blijft de bestaande pvKwh parsing niet alleen behouden, maar wordt die ook expliciet meegenomen in sizing, simulatie en rapportage.

Bronnen:
- lib/parsing.ts::mapRows
- lib/calculations.ts::derivePvIntervalFlow
- lib/simulation.ts::simulatePvScenario

### 11.2 PV sizing rule
De PV-dimensionering gebruikt computePvSizing.

De regels zijn:
- benodigde kWh komt uit het maximale opgebouwde PV-surplus binnen een dag
- benodigde kW komt uit de hoogste kwartier-mismatch tussen PV en load
- daarna worden, net als bij peak shaving, compliance, efficiency en safety factor toegepast
- productselectie gebeurt met selectMinimumCostBatteryOptions(requiredKwh, requiredKw)

Daardoor blijft de productkeuze consistent met de bestaande batterijcatalogus:
- aanbevolen product = goedkoopste haalbare configuratie
- alternatief = tweede geldige optie
- 
oFeasibleBatteryByPower blijft actief als de vermogenseis niet haalbaar is

Bronnen:
- lib/calculations.ts::computePvSizing
- lib/calculations.ts::selectMinimumCostBatteryOptions

### 11.3 PV simulation behavior
De PV-simulatie loopt interval voor interval door de dataset:
1. directe load wordt eerst door PV gedekt
2. overblijvend PV-surplus laadt de batterij, begrensd door:
- resterende SOC-capaciteit
- maxChargeKw
- charge efficiency
3. daarna ontlaadt de batterij om resterende load te dekken, begrensd door:
- beschikbare SOC
- maxDischargeKw
- discharge efficiency

Gemeten outputs zijn onder andere:
...
Bronnen:
- lib/simulation.ts::simulatePvScenario
- lib/simulation.ts::simulateAllPvScenarios
- lib/simulation.ts::buildPvSummaryFromScenario